...
// Create a new API key. The response carries the cleartext key exactly once —
// only its hash is persisted, so it cannot be retrieved again afterwards.
export async function createApiKey(
  data: ApiKeyFormData,
): Promise<ApiResponse<{ id: number; key: string }>> {
